        ...
        <header class="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sm md:hidden">
            <div class="flex items-center gap-3 px-4 py-3">
                ...
                <div class="search-bar flex items-center flex-1 px-4 py-2 gap-2">
                    ...
                    <input type="text" placeholder="Cari kategori..." class="bg-transparent text-sm text-gray-700 placeholder-gray-400 outline-none flex-1">
                </div>
        ...
        <header class="hidden md:flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100 shadow-sm sticky top-0 z-30">
            <div class="flex items-center gap-8">
                ...
                <nav class="flex gap-6">
                    ...
                    <a href="{{ route('category') }}" class="text-sm font-semibold text-cyan-600 border-b-2 border-cyan-600 pb-0.5">Kategori</a>
                    <a href="{{ route('restaurant.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-800 transition">Restoran</a>
                </nav>
            </div>
        ...
        <main class="px-4 md:px-8 mt-6">
            ...
            <section class="mb-8">
                ...
                <div class="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
                    <a href="{{ route('category') }}" class="cat-pill {{ !isset($category) ? 'active' : '' }} flex-shrink-0 flex flex-col items-center gap-2 bg-white border border-gray-100 rounded-2xl px-5 py-3 shadow-sm min-w-24">
                        <div class="icon-container w-10 h-10 bg-cyan-50 rounded-xl flex items-center justify-center transition-colors">
                            <span class="iconify text-cyan-600" data-icon="mdi:shape-outline" data-width="20"></span>
                        </div>
                        <h3 class="text-gray-700 text-xs font-semibold text-center leading-tight transition-colors">Semua</h3>
                    </a>
                    
                    @foreach ($categories as $cat)
                    <a href="{{ route('category.products', $cat->id) }}" class="cat-pill {{ isset($category) && $category->id == $cat->id ? 'active' : '' }} flex-shrink-0 flex flex-col items-center gap-2 bg-white border border-gray-100 rounded-2xl px-5 py-3 shadow-sm min-w-24">
                        <div class="icon-container w-10 h-10 bg-cyan-50 rounded-xl flex items-center justify-center transition-colors">
                            <span class="iconify text-cyan-600" data-icon="{{ $cat->icon }}" data-width="20"></span>
                        </div>
                        <h3 class="text-gray-700 text-xs font-semibold text-center leading-tight transition-colors">{{ $cat->name }}</h3>
                    </a>
                    @endforeach
                </div>
            </section>
