...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="max-w-4xl mx-auto p-4">
        <form id="checkout-form">
            ...
            <div class="bg-white rounded-lg shadow-md p-4 mb-4">
                ...
                <div class="flex items-center justify-between py-2 border-b border-gray-200 last:border-b-0">
                    <div class="flex items-center space-x-3">
                        <img src="/assets/pasar-ikan.png" alt="{{ $product->name }}"
                            class="w-12 h-12 object-cover rounded">
                        <div>
                            <h3 class="font-medium">{{ $product->name }}</h3>
                            <input type="number"
                                class="w-16 px-2 py-1 border border-gray-300 rounded text-sm text-gray-500"
                                min="1" name="quantity" id="quantity" value="{{ request('quantity', 1) }}">
                        </div>
                    </div>
                    <span class="font-semibold text-red-500">Rp{{ number_format($product->price, 0, ',', '.') }}</span>
                </div>
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-gray-600">Subtotal</span>
                        <span class="font-semibold"
                            id="subtotal">Rp{{ number_format($product->price * request('quantity', 1), 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-gray-600">Ongkir</span>
                        <span class="font-semibold">Rp{{ number_format($product->shipping_cost, 0, ',', '.') }}</span>
                    </div>
                    <hr class="my-2">
                    <div class="flex justify-between items-center">
                        <span class="text-lg font-bold">Total</span>
                        <span class="text-lg font-bold text-red-500"
                            id="total-display">Rp{{ number_format($product->price * request('quantity', 1), 0, ',', '.') }}</span>
                        <input type="hidden" name="total"
                            value="{{ $product->price * request('quantity', 1) }}"
                            id="total">
                    </div>
                </div>
            </div>
            ...
            <script>
                // Auto-update subtotal and total when quantity changes
                // $(function() {
