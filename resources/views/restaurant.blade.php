...
<body class="bg-gray-50 min-h-screen pb-24">
    <div class="max-w-7xl mx-auto">
        ...
        <header
            ...
            <div class="flex items-center gap-8">
                ...
                <nav class="flex gap-6">
                    ...
                    <a href="{{ route('restaurant.index') }}"
                        class="text-sm font-semibold text-cyan-600 border-b-2 border-cyan-600 pb-0.5">Restoran</a>
                </nav>
            </div>

            <div class="flex items-center gap-4">
                <form method="GET" action="{{ route('restaurant.show', $restaurant->id) }}"
                    class="search-bar flex items-center w-72 px-4 py-2 gap-2">

                    <span class="iconify text-gray-400" data-icon="mdi:magnify" data-width="18"></span>

                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari di restoran ini..."
        ...
        <main class="md:px-8 mt-4 md:mt-8 mb-10">
            {{-- RESTO HERO --}}
            <section
                class="max-w-7xl mx-auto rounded-none md:rounded-3xl overflow-hidden shadow-xl mb-8 border border-gray-100 bg-white">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-6 md:p-10 flex items-end justify-between">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span
                                    class="text-xs font-semibold text-emerald-600 bg-emerald-50/90 backdrop-blur-sm px-2.5 py-1 rounded-lg flex items-center gap-1.5 shadow-sm">
                                    <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                                    Buka Sekarang
                                </span>
                                <span
                                    class="bg-white/20 backdrop-blur-md text-white text-xs font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1">
                            ...
                            <p class="text-white/80 text-sm md:text-base flex items-center gap-1.5">
                                <span class="iconify flex-shrink-0" data-icon="mdi:map-marker-outline"
                                    data-width="18"></span>
                                {{ $restaurant->address }}
                            </p>
                        </div>
                        ...
                        <div class="hidden md:flex gap-3">
                            <button
                                class="bg-white/20 hover:bg-white/30 backdrop-blur-md text-white w-12 h-12 rounded-xl flex items-center justify-center transition">
                ...
                <div class="px-6 py-4 flex flex-wrap gap-6 bg-white shrink-0 border-t border-gray-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $restaurant->open_time ?? '08:00' }} -
                                {{ $restaurant->close_time ?? '22:00' }}</p>
                        </div>
                    </div>
            ...
            <section class="mb-10 px-4 md:px-0">
                ...
                @if($products->isEmpty())
                    <div class="text-center py-12 bg-white rounded-3xl border border-gray-100 shadow-sm mx-4 md:mx-0">
                        <span class="iconify mx-auto text-gray-300 mb-4" data-icon="mdi:flask-empty-outline"
                            data-width="64"></span>
